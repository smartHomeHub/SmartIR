...
| `power_sensor_restore_state` | boolean | optional | If `power_sensor` is set, and the device is likely to decrease power consumption bellow `power_sensors` threshold (for instance, a minisplit cycling compressor on and off while in heating or cooling mode), setting this to `true` will cause the climate entity state to update dynamically between OFF and last known ON state, following the state of the `power_sensor`. |
